...
const ChangeIndicator = ({ value, isPositive }) => {
  return (
    <div
      className={`flex items-center gap-1 ${isPositive ? "text-green-60" : "text-red-60"}`}
    >
      {isPositive ? (
  ...
  return (
    <div className="flex items-center justify-between py-3">
      <div className="flex items-center gap-3">
        <div
          className={`w-8 h-8 rounded-full ${iconColor} flex items-center justify-center text-white font-bold text-xs`}
        >
          {symbol[0]}
        </div>
        <div>
...
export default function MarketStatsPage() {
  ...
  const ChartSvgBlock = ({ isRed }) => {
    ...
    return (
      <div
        className="w-full flex-col mt-8"
        style={{ width: "100%", height: "150px" }}
      >
        <div
          className="relative w-full h-[120px] max-w-[502px] overflow-x-auto overflow-y-hidden"
        ...
        >
          ...
          >
            ...
            >
              <div
                className={`w-px h-[120px] border-l-2 border-dashed ${isRed ? "border-red-60" : "border-green-60"}`}
              ></div>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 w-full pt-16">
        <div className="max-w-[1400px] mx-auto px-6 py-8">
          <h1 className="text-display-3 text-gray-100 mb-2">
            Market statistics
          </h1>
        </div>
