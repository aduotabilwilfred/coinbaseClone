...
const CRYPTO = [
  ...
  },
  {
    ...
    mcapUsd: 82.9e9,
    volUsd: 1.4e9,
    img: "https://assets.coingecko.com/coins/images/44/small/xrp-symbol-white-128.png",
  },
  {
    symbol: "USDCUSDT",
    name: "USDC",
...
const PERPS = [
  ...
  },
  {
    ...
    volUsd: 1.9e6,
    fund: "0.0057%",
    img: "https://assets.coingecko.com/coins/images/9519/small/paxg.PNG",
  },
  {
    name: "PEPE PERP",
    ticker: "PEPE",
    ...
    volUsd: 1.2e6,
    fund: "-0.0012%",
    img: "https://assets.coingecko.com/coins/images/29850/small/pepe-token.jpeg",
  },
  {
    name: "SUI PERP",
    ticker: "SUI",
...
const StockIcon = () => (
  ...
  >
    ...
    />
    <path
      d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm-1-6h2v2h-2zm0-8h2v6h-2z"
      fill="currentColor"
    />
  </svg>
);
...
const CryptoRow = ({ item, prices, ghsRate, query }) => {
  ...
  return (
    <a
      href="#"
      className="flex items-center gap-3 px-4 py-3 hover:bg-gray-5 transition-colors group"
    >
      <img
        src={item.img}
        alt={item.ticker}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-[0.875rem] font-semibold text-gray-100">
            {item.name}
          </span>
          {item.rank && (
            <span className="text-[0.6875rem] font-medium text-gray-60 bg-gray-10 px-1.5 py-0.5 rounded-md">
              #{item.rank}
            </span>
          )}
        </div>
      ...
      <div className="text-right shrink-0 mr-8">
        <p className="text-[0.8125rem] text-gray-60">
          GHS {fmtAmt(volGhs)} vol
        </p>
        <p className="text-[0.8125rem] text-gray-60">
          GHS {fmtAmt(mcapGhs)} mcap
        </p>
      </div>
      <div className="text-right shrink-0 min-w-[120px]">
        <p className="text-[0.875rem] font-semibold text-gray-100">
          GHS {fmtPrice(priceGhs)}
        </p>
        <p
          className={`text-[0.8125rem] font-medium ${up ? "text-green-600" : "text-red-500"}`}
        >
...
const StockRow = ({ item, ghsRate, query }) => {
  ...
  return (
    <a
      href="#"
      className="flex items-center gap-3 px-4 py-3 hover:bg-gray-5 transition-colors"
    >
    ...
    async function fetchData() {
      try {
        const [tickerRes, fxRes] = await Promise.all([
          fetch(
            `https://api.binance.com/api/v3/ticker/24hr?symbols=${JSON.stringify(BINANCE_SYMBOLS)}`,
          ),
          fetch(
      ...
      <div className="w-full max-w-[1228px] mx-auto px-8 lg:px-12 pointer-events-none">
        <div className="ml-auto w-full max-w-[680px] bg-white rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden pointer-events-auto">
          ...
          <div className="max-h-[480px] overflow-y-auto">
            {activeTab === "Top" && (
              <>
                <p className="px-4 pt-4 pb-1 text-[0.6875rem] font-semibold text-gray-60 tracking-wider uppercase">
                  Crypto
                </p>
                ...
                <p className="px-4 py-3 text-[0.8125rem] text-gray-60">
                  Coming soon
                </p>
              </>
            )}
            ...
            {activeTab === "Predictions" && (
              <p className="px-4 py-8 text-center text-[0.8125rem] text-gray-60">
                Coming soon
              </p>
            )}
