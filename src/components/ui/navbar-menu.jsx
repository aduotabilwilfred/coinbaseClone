...
export const NavDropdown = ({ children, isOpen }) => (
  <AnimatePresence>
    {isOpen && (
      <div className="absolute top-full left-0 w-full bg-white border-t border-gray-10 shadow-[0_8px_24px_rgba(0,0,0,0.08)] z-50">
        {children}
      </div>
    )}
  </AnimatePresence>
...
export const NavMenuItem = ({ icon, title, description, href = "#" }) => (
  ...
  >
    ...
    <div className="min-w-0">
      <p className="text-[0.875rem] font-semibold text-gray-100 leading-5">
        {title}
      </p>
      {description && (
        <p className="text-[0.8125rem] text-gray-60 leading-[1.35] mt-0.5">
          {description}
        </p>
      )}
    </div>
...
export const NavFeatured = ({
  ...
  <div className="flex flex-col gap-4 min-w-[220px] max-w-[260px]">
    <div className="w-full aspect-square max-w-[180px] rounded-2xl overflow-hidden bg-blue-60 shrink-0">
      {image ?? (
        <div className="w-full h-full bg-gradient-to-br from-[#EA580C] to-[#1A6FFF]" />
      )}
    </div>
    <div className="flex flex-col gap-1">
      <p className="text-[1rem] font-semibold text-gray-100 leading-6">
        {title}
      </p>
