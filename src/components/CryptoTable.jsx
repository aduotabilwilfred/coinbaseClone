...
/* ── Animated price: only flips the digits that actually changed ──
   Strategy:
   - useEffect keeps prevValue in sync AFTER each render, so
     during the current render prevValue still holds the OLD string.
...
const AnimatedPrice = ({ value, dir }) => {
	...
	// Update after render so next render gets current as "previous"
	useEffect(() => {
		setPrevValue(value);
	}, [value]);
